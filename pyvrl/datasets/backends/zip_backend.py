...
class ZipItem(object):
    ...
    def __init__(self, video_info: dict, zip_fmt: str, frame_fmt: str):
        ...
        self.dataIndex = video_info['index']
    ...
    def get_frame_with_index(self, indices: Union[List[int], List[List[int]]]) -> Tuple[Union[List[np.ndarray], List[List[np.ndarray]]], List[int]]:
        """ Load image frames from the given zip file as well as the data index.
        Args:
            indices: frame index list (0-based index)
        Returns:
            img_list: the loaded image list, each element is a np.ndarray in shape of [H, W 3]
        ...
        if isinstance(indices[0], list):
            for clip_list in indices:
                this_list = []
                for idx in clip_list:
                    file_name = self.frame_fmt.format(int(idx) + 1)
                    img = self.load_image_from_zip(self.frame_zip_fid, file_name, cv2.IMREAD_COLOR)
                    this_list.append(img)
                img_list.append(this_list)
        else:
            for idx in indices:
                file_name = self.frame_fmt.format(int(idx) + 1)
                img = self.load_image_from_zip(self.frame_zip_fid, file_name, cv2.IMREAD_COLOR)
        ...
        return img_list, [self.dataIndex]
