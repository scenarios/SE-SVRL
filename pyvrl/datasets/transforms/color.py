...
class RandomColorJitter(BaseTransform):
    ...
    def __init__(self,
                 ...
                 prob = 0.5):
        self.prob = prob
        self.proxy_transform = ColorJitter(brightness=brightness, contrast=contrast, saturation=saturation, hue=hue)

    def get_transform_param(self, data, *args, **kwargs) -> dict:
        flag = (0 < np.random.rand() < self.prob )
        return dict(flag=flag, delta=None)

    def _apply_image(self,
                     data: torch.Tensor,
                     transform_param: dict):
        if transform_param['flag']:
            data = self.proxy_transform(data)
...
class RandomHueSaturation(BaseTransform):
    ...
    def get_transform_param(self, *args, **kwargs) -> dict:
        flag = (0 < np.random.rand() < self.prob)
        hue_delta = np.exp(np.random.uniform(-self.hue_delta, self.hue_delta))
