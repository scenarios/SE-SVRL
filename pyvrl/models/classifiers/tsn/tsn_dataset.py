...
class TSNDataset(Dataset):
    ...
    def evaluate(self, results, logger=None):
        ...
        assert len(results) == len(self), f'The results should have same size as gts. But' \
                                          f' got {len(results)} and {len(self)}'
        labels = np.array([int(self.data_source[_]['label']) - 1 for _ in range(len(self))], np.long)
        sort_inds = results.argsort(axis=1)[:, ::-1]

        acc_dict = dict()
        for k in [1, 5]:
            top_k_inds = sort_inds[:, :k]
            correct = (top_k_inds.astype(np.long) == labels.reshape(len(self), 1))
