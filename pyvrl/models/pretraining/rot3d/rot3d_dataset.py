...
class RotateDataset(TSNDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # find the index of rotation transformation in the list of image transformations,
        # because we need to get the rotation degree as the ground-truth.
        try:
            self.rot_trans_index = next(i for i, trans in enumerate(self.img_transform.transforms)
                                        if trans.__class__.__name__ == 'GroupRectRotate')
        except Exception:
            logger = logging.getLogger()
            logger.error("Cannot find 'GroupRectRotate' in the image transformation configuration."
    ...
    def __getitem__(self, idx):
        ...
        if not self.test_mode:
            gt_label = torch.LongTensor([trans_params[self.rot_trans_index]['flag']])
