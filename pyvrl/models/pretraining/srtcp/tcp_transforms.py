...
class RandomRegionSampler(object):
    ...
    def __init__(self,
        ...
        self.anchors = np.concatenate((widths.reshape(-1, 1), heights.reshape(-1, 1)), axis=-1)

    def sample(self, data):
        """ Sample 'self.num_rois' boxes.
        Args:
            data (list): image list, each element is a numpy.ndarray in shape of [H, W, 3]
        """
        h, w = data[0].shape[0:2]

        # random sample box shapes
        anchor_inds = np.random.randint(0, len(self.anchors), size=(self.num_rois, ))
        box_shapes = self.anchors[anchor_inds].copy()
        if self.scale_jitter is not None:
            scale_factors = np.random.uniform(-self.scale_jitter, self.scale_jitter, size=(self.num_rois, 2))
        ...
        boxes = np.concatenate((x1.reshape(-1, 1),
                                ...
                                (y1 + box_shapes[:, 1]).reshape(-1, 1)), axis=1)
...
class PatchMask(BaseTransform):
    ...
    def pick_patches(self,
        ...
        patches = []
        for frame_idx in range(traj_rois.shape[0]):
            w, h = traj_sizes[frame_idx]
            patch = cv2.resize(img, (w, h))
    ...
    def pick_alphas(self,
        ...
        base_x_grids, base_y_grids = np.meshgrid(np.arange(base_w).astype(np.float32),
    ...
    def _apply_image(self,
        ...
        transformed_data = []
        sizes = transform_param['traj_rois'][..., 2:4] - transform_param['traj_rois'][..., 0:2]
        ...
        for frame_idx in range(len(data)):
            ...
            for patch_idx in arg_rank:
                ...
                img[y1:y2, x1:x2, :] = img[y1:y2, x1:x2, :] * (1 - i_alpha) + i_patch * i_alpha
    ...
    def rectangle_movement(boxes: np.ndarray,
        ...
        Args:
            ...
            key_frame_probs (float): probability distribution of how many key frames will be sampled.
