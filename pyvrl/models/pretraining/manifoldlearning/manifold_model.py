...
class MANIFOLD(nn.Module):
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys, dataInds):
        # gather keys before updating queue
        keys = concat_all_gather(keys)
        dataInds = concat_all_gather(dataInds)

        batch_size = keys.shape[0]

        ptr = int(self.queue_ptr)
        assert self.K % batch_size == 0  # for simplicity
        assert batch_size == dataInds.shape[0]

        # replace the keys at ptr (dequeue and enqueue)
        self.queue[:, ptr:ptr + batch_size] = keys.transpose(0, 1)
        self.indQueue[ptr:ptr + batch_size] = dataInds.squeeze()
    ...
    @torch.no_grad()
    def _batch_shuffle_ddp(self, k_p, k_n):
        """
        Batch shuffle, for making use of BatchNorm.
        *** Only support DistributedDataParallel (DDP) model. ***
        """
        # gather from all gpus
        batch_size_this = k_p.shape[0]
        k_p_gather = concat_all_gather(k_p)
        k_n_gather = concat_all_gather(k_n)
        batch_size_all = k_p_gather.shape[0]
        ...
        return k_p_gather[idx_this], k_n_gather[idx_this], idx_unshuffle

    @torch.no_grad()
    def _batch_unshuffle_ddp(self, k_p, k_n, idx_unshuffle):
        """
        Undo batch shuffle.
        *** Only support DistributedDataParallel (DDP) model. ***
        """
        ...
        return k_p_gather[idx_this], k_n_gather[idx_this]

    @torch.no_grad()
    def _remove_same_instance(self, cur_dataInd):
        pos = self.indQueue[:,None] == cur_dataInd
        pos = torch.sum(pos.int(), dim=-1) - 1
        pos = torch.nonzero(pos, as_tuple = True)
    ...
    def forward(self,
                ...
                gt_trajs: torch.Tensor,
                clip_q: torch.Tensor,
                clip_k_p: torch.Tensor,
                clip_k_n: torch.Tensor,
                dataIndex: torch.Tensor):
        ...
        q = self.backbone(clip_q)  # [N, C, t, h, w]
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            #clip_k_p, clip_k_n, idx_unshuffle = self._batch_shuffle_ddp(clip_k_p, clip_k_n)

            k_p = self.key_encoder(clip_k_p)  # keys: NxC
            k_p = self.k_fc(k_p.view(k_p.shape[0:2] + (-1,)).mean(dim=-1))
            k_p = nn.functional.normalize(k_p, dim=1)

            k_n = self.key_encoder(clip_k_n)  # keys: NxC
            k_n = self.k_fc(k_n.view(k_n.shape[0:2] + (-1,)).mean(dim=-1))
            k_n = nn.functional.normalize(k_n, dim=1)

            # undo shuffle
            #k_p, k_n = self._batch_unshuffle_ddp(k_p, k_n, idx_unshuffle)
        ...
        # negative logits: Nx1
        l_neg = torch.einsum('nc,nc->n', [q, k_n]).unsqueeze(-1)
        # negative logits from queue: NxK
        # remove same instances first
        modified_queue = self._remove_same_instance(cur_dataInd=dataIndex.squeeze())
        #modified_queue = self.queue.clone().detach()
        l_neg_queue = torch.einsum('nc,ck->nk', [q, modified_queue])

        # Manifold Learning
        # logits: Nx(1+1+K)
        logits = torch.cat([l_pos, l_neg, l_neg_queue], dim=1)
        # MoCo
        # logits: Nx(1+K)
        #logits = torch.cat([l_pos, l_neg_queue], dim=1)
        ...
        # dequeue and enqueue
        self._dequeue_and_enqueue(k_p, dataIndex)

        losses = dict(list(tcp_losses.items()) + list(moco_losses.items()))
        #losses = dict(list(moco_losses.items()))
